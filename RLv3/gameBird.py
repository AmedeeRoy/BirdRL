...
class PlayerBird:
    ...
    def getScore(self):
        ...
        if dead:
            ...
            score = self.nbFish*self.reward["win"]
        return score

    def game_over(self):
        dead = (self.getLife() <= 0)
        home = (self.init_pos == (self.row, self.col))
        satiated = (self.nbFish > 0)
    ...
    def drawProperties(self, screen, gameMap):
        ...
        t = font.render(str(self.nbFish), True, WHITE, BLACK)
    ...
    def visibleMap(self, gameMap):
        ...
        minrow = self.row
        maxrow = self.row + 2 * self.visibility
        mincol = self.col
        maxcol = self.col + 2 * self.visibility
